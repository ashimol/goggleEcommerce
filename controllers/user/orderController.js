  ...
  try {
    ...
    if (usedCoupon) {
      return res
        .status(400)
        .json({
          message: "Coupon has already been used. Please try another one.",
        });
    }
    ...
    if (cartTotal < coupon.minPurchaseAmount) {
      return res
        .status(400)
        .json({
          message: `Minimum purchase amount is ${coupon.minPurchaseAmount} for this coupon.`,
        });
    }
    ...
    if (cartTotal > coupon.maxPurchaseAmount) {
      return res
        .status(400)
        .json({
          message: `Maximum purchase amount is ${coupon.maxPurchaseAmount} for this coupon.`,
        });
    }
    ...
    const baseDiscount = cart.items.reduce((total, item) => {
      return (
        total +
        (item.productId.regularPrice - item.productId.salePrice) * item.quantity
      );
    }, 0);
...
const placeOrder = async (req, res) => {
  try {
    ...
    if (
      !["Online Payment", "Cash On Delivery", "WalletPayment"].includes(
        paymentMethod
      )
    ) {
      ...
      for (const cartItem of cart.items) {
        if (paymentMethod !== "Online Payment") {
          const orderItem = items.find(
            (order) =>
              order.productId.toString() === cartItem.productId.toString()
          );
          if (orderItem) {
    ...
    if (razorpay_signature === expectedSign) {
      ...
      if (order) {
        ...
        for (const orderItem of order.items) {
          const productInOrder = await Product.findById(orderItem.productId);
          if (productInOrder) {
            const newQuantity = productInOrder.quantity - orderItem.quantity;
      ...
      if (order) {
        console.log(
          "Updating order status to Pending for failed payment:",
          order._id
        );
...
const downloadInvoice = async (req, res, next) => {
  try {
    ...
    doc.text("Shipping Address:", leftColumnX, shippingAddressStartY, {
      underline: true,
    });
    ...
    doc.text(
      `${order.addressId.city}, ${order.addressId.state} - ${order.addressId.pin}`,
      leftColumnX,
      shippingAddressStartY + 60
    );
    doc.text(
      `Phone: ${order.addressId.contactNo}`,
      leftColumnX,
      shippingAddressStartY + 80
    );
    ...
    doc.text("Brand", 50 + columnWidths.name, tableTop, {
      width: columnWidths.brand,
      ellipsis: true,
    });
    doc.text("Qty", 50 + columnWidths.name + columnWidths.brand, tableTop, {
      width: columnWidths.qty,
    });
    doc.text(
      "Amount",
      50 + columnWidths.name + columnWidths.brand + columnWidths.qty,
      tableTop,
    ...
    doc.text(
      ...
      tableTop,
      { width: columnWidths.taxableValue }
    );
    doc.text(
      "Total",
      50 +
    ...
    doc.text(selectedItem.productId.productName, 50, tableRow, {
      width: columnWidths.name,
      ellipsis: true,
    });
    doc.text(
      selectedItem.productId.brand.name,
      50 + columnWidths.name,
      tableRow,
      { width: columnWidths.brand, ellipsis: true }
    );
    doc.text(
      selectedItem.quantity.toString(),
      50 + columnWidths.name + columnWidths.brand,
      tableRow,
      { width: columnWidths.qty }
    );
    doc.text(
    ...
    const discount = selectedItem.price - selectedItem.price * 0.9; 
    doc.text(
      discount.toFixed(2),
      50 +
        columnWidths.name +
    ...
    const taxableValue = selectedItem.price * 0.9; 
    ...
    doc.text(
      (taxableValue * selectedItem.quantity).toFixed(2),
      50 +
        columnWidths.name +
        columnWidths.brand +
        columnWidths.qty +
